...
В подсистеме «Список заказов» собраны все созданные заказы, приказы и наряды в системе. 

{% note info %}

Список приказов и нарядов визуально и по набору типовых задач аналогичен списку заказов, поэтому рассмотрим работу только со списком заказов.
...
Для переключения между режимами: зажмите Alt на клавиатуре и прокручивайте колесо мыши;
- **месяц**, по которому строится диаграмма. 
Для переключения между месяцами: зажмите Shift на клавиатуре и прокручивайте колесо мыши;
- **заказ**, по которому отображаются задачи.

Кроме того, можно кликнуть на задачу и увидеть всплывающее окно со статусом, номером, датами и наименованием пространства. 
...
| ![task_3](_images/_screen_task_late_start_end.png){.center height=40} | Задача выполнена в срок, но началась позже запланированного | 
...
![task_from_1_to_3](_images/_screen_task_from_1_to_3.png){.center width=600}

- для задач типа «наряд» — количество подзадач: по клику скрытые задачи развернутся вниз на диаграмме   
![task_1_in_6](_images/_screen_task_1_6.png){.center width=500}
...
Если одновременно выполняется много задач, часть из них отображается на диаграмме, остальные сворачиваются. Развернуть все задачи можно по клику на число скрытых задач справа по строке.  
